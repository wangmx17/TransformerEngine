...
std::tuple<at::Tensor, at::Tensor, at::Tensor> fused_topk_with_score_function_fwd(
  ...
  nvte_fused_topk_with_score_function_forward(
      ...
      0, expert_bias_cu.data(), probs_cu.data(),
...
at::Tensor fused_topk_with_score_function_bwd(int num_tokens, int num_experts,
  ...
  at::Tensor grad_logits = at::empty(
      {num_tokens, num_experts}, at::dtype(intermediate_output.scalar_type()).device(at::kMUSA));
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> fused_score_for_moe_aux_loss_fwd(
  ...
  at::Tensor routing_map =
      at::empty({num_tokens, num_experts}, at::dtype(at::kBool).device(at::kMUSA));
  // For softmax, and for sigmoid when topk == 1, the backward input matches the final scores.
  // Reuse the same storage to avoid an extra full-matrix write in forward.
  const bool separate_intermediate_output = (score_function == "sigmoid" && topk > 1);
  at::Tensor intermediate_output =
      separate_intermediate_output
          ? at::empty({num_tokens, num_experts}, at::dtype(logits.scalar_type()).device(at::kMUSA))
          : scores;
